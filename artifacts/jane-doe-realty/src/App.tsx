...
import {
  ...
  Instagram, Youtube, Send, Play, Pause, Volume2, VolumeX, MessageCircle
} from "lucide-react";
...
const FALLBACK_AGENT = {
  ...
  instagram: "https://www.instagram.com/mel.castanares?igsh=M2J5eTVqdnhyZ2sx",
  ...
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    @media(max-width:640px) {
      ...
      /* Featured blog hero: taller on mobile so text is readable */
      .blog-featured-hero { aspect-ratio: 4/3 !important; min-height: 280px !important; }

      /* Hero CTA buttons: stack on very small screens */
      .hero-cta-row { flex-direction: column !important; align-items: stretch !important; }
      .hero-cta-row a, .hero-cta-row button { justify-content: center !important; text-align: center !important; }

      /* Testimonials & property cards: full width on mobile */
      .card-grid-340 { grid-template-columns: 1fr !important; }

      /* Instagram blog section: stack on mobile */
      .ig-split { grid-template-columns: 1fr !important; }
    ...
    <div>
      ...
      <section style={{ position: "relative", width: "100%", height: "100vh", minHeight: 700, overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <div style={{ position: "relative", zIndex: 2, textAlign: "center", padding: "0 24px", maxWidth: 860, marginTop: 40 }}>
          ...
          <div className="hero-text-anim hero-text-anim-d4 hero-cta-row" style={{ display: "flex", gap: 14, justifyContent: "center", flexWrap: "wrap" }}>
        ...
        {(propsResp?.properties || []).filter(p => p.featured && p.type !== "rental" && p.price >= 10000).length > 0 ? (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 20 }}>
            {[
              ...
              const inner = (
                <>
                  <img src={item.img} alt="" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", display: "block", transition: "transform 0.5s ease" }} />
                  <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to top, rgba(5,8,15,0.92) 0%, rgba(5,8,15,0.45) 50%, rgba(5,8,15,0.15) 100%)" }} />
                  <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "18px 18px 20px" }}>
                    <div style={{
                      ...
                      <div style={{ color: "#fff", fontWeight: 700, fontSize: 14, fontFamily: "'DM Serif Display', Georgia, serif", marginBottom: 4, lineHeight: 1.25 }}>{item.label}</div>
                      <div style={{ color: "rgba(255,255,255,0.65)", fontSize: 11, marginBottom: 10 }}>{item.sub}</div>
                      <div style={{ display: "flex", alignItems: "center", gap: 5, color: BRAND.tealLight, fontSize: 11, fontWeight: 700, letterSpacing: "0.05em", textTransform: "uppercase" }}>
  ...
  return (
    <div style={{ paddingTop: 120 }}>
      ...
      <div className="section-pad" style={{ paddingTop: 40 }}>
        ...
        <Reveal>
          <div style={{ display: "flex", gap: 0, marginBottom: 32, borderBottom: `1px solid ${BRAND.border}` }}>
            {(["sale", "rental"] as const).map(t => (
              <button key={t} onClick={() => { setTab(t); setSearchCity(""); setFilter({ minPrice: "", maxPrice: "", beds: "" }); }} style={{
        ...
        {/* Filter bar — only shown on Rentals tab */}
        {tab === "rental" && (
          <Reveal delay={0.05}>
            <div style={{ display: "flex", gap: 12, marginBottom: 36, flexWrap: "wrap", alignItems: "flex-end" }}>
              ...
              <div style={{ flex: "0 0 140px" }}>
                <label style={{ fontSize: 11, color: BRAND.textDim, letterSpacing: "0.1em", textTransform: "uppercase", display: "block", marginBottom: 6 }}>Max Rent</label>
                <select className="input-custom" value={filter.maxPrice} onChange={e => setFilter(f => ({ ...f, maxPrice: e.target.value }))}>
                  <option value="">Any</option>
                  {[1500,2000,2500,3000,3500,4000,5000].map(v => (
              ...
              {(searchCity || filter.maxPrice || filter.beds) && (
        ...
        {tab === "rental" && (
          <div>
            ...
            {!rentalsLoading && !rentalsError && filteredRentals.length === 0 && (
              <Reveal>
                <div style={{ background: BRAND.bgCard, border: `1px dashed ${BRAND.border}`, padding: "60px 40px", textAlign: "center" }}>
                  <h3 className="font-display" style={{ fontSize: 26, marginBottom: 10 }}>
                    {rentals.length > 0 ? "No Rentals Match Your Filters" : "No Rentals Available Right Now"}
                  </h3>
                  <p style={{ color: BRAND.textMuted, fontSize: 14, marginBottom: 24 }}>
                    {rentals.length > 0 ? "Try clearing the filters to see all available rentals." : "New properties are added regularly. Check AppFolio for the latest or reach out to Mel."}
                  </p>
              ...
              <>
                <div style={{ background: `${BRAND.teal}0F`, border: `1px solid ${BRAND.teal}30`, padding: "12px 18px", marginBottom: 28, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <div style={{ width: 8, height: 8, borderRadius: "50%", background: BRAND.teal }} />
                    <span style={{ fontSize: 13, color: BRAND.teal, fontWeight: 600 }}>
                      Live from AppFolio · {filteredRentals.length} propert{filteredRentals.length === 1 ? "y" : "ies"} available
                      {filteredRentals.length < rentals.length && <span style={{ fontWeight: 400, color: BRAND.textDim, fontSize: 11 }}> (filtered from {rentals.length})</span>}
                    </span>
                  </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 64, alignItems: "center" }}>
          ...
          <Reveal direction="left" delay={0.2}>
            ...
            <div style={{ display: "flex", gap: 12, marginTop: 32, flexWrap: "wrap" }}>
              ...
              <a href="https://www.instagram.com/mel.castanares?igsh=M2J5eTVqdnhyZ2sx" target="_blank" rel="noopener noreferrer" className="btn-outline" style={{ display: "inline-flex", alignItems: "center", gap: 8, textDecoration: "none", fontSize: 11, padding: "14px 24px" }}>
                <Instagram size={14} /> @mel.castanares
              </a>
            </div>
          </Reveal>
        ...
        <Reveal delay={0.2}>
          <div style={{ marginTop: 80, paddingTop: 56, borderTop: `1px solid ${BRAND.border}` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 40, flexWrap: "wrap", gap: 16 }}>
              ...
              <a href="https://www.instagram.com/mel.castanares?igsh=M2J5eTVqdnhyZ2sx" target="_blank" rel="noopener noreferrer" className="btn-outline" style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "10px 20px", fontSize: 11, textDecoration: "none" }}>
                <Instagram size={14} /> @mel.castanares
              </a>
            </div>
            {/* Instagram — split: embedded reel on left, profile card on right */}
            <div className="ig-split" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 24, alignItems: "start" }}>
              ...
              <div style={{ background: BRAND.bgCard, border: `1px solid ${BRAND.border}`, borderRadius: 16, overflow: "hidden", boxShadow: "0 8px 32px rgba(0,0,0,0.07)" }}>
                <div style={{ background: "#000", padding: "10px 16px", display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <span style={{ color: "#fff", fontSize: 13, fontWeight: 600 }}>@mel.castanares</span>
                </div>
              ...
              {/* Profile card */}
              <a href="https://www.instagram.com/mel.castanares?igsh=M2J5eTVqdnhyZ2sx" target="_blank" rel="noopener noreferrer" style={{ textDecoration: "none", display: "block" }}>
                <div style={{
                  background: BRAND.bgCard, border: `1px solid ${BRAND.border}`,
                ...
                >
                  ...
                  <div style={{ padding: "44px 24px 28px" }}>
                    <div style={{ fontWeight: 700, fontSize: 16, color: BRAND.text, marginBottom: 2 }}>mel.castanares</div>
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, background: "linear-gradient(135deg, #833ab4, #fd1d1d, #fcb045)", color: "#fff", padding: "11px 20px", borderRadius: 8, fontSize: 12, fontWeight: 600 }}>
                      <Instagram size={14} /> Follow @mel.castanares
                    </div>
                  </div>
                </div>
              </a>
            ...
            <p style={{ color: BRAND.textDim, fontSize: 13, marginTop: 20, textAlign: "center" }}>
              Market updates, listings, and O'ahu life — follow <a href="https://www.instagram.com/mel.castanares?igsh=M2J5eTVqdnhyZ2sx" target="_blank" rel="noopener noreferrer" style={{ color: BRAND.teal, textDecoration: "none", fontWeight: 600 }}>@mel.castanares</a> on Instagram.
            </p>
          </div>
        </Reveal>
      </div>
...
const BLOG_IG_EMBEDS = {
  ...
  "i-toured-50-open-houses-so-you-dont-have-to": [
    { url: "https://www.instagram.com/reel/DWQWkHZgg2j/embed", caption: "Behind the scenes at open houses" },
  ],
};
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 48 }}>
          <Reveal direction="right">
            ...
            {[
              ...
              { icon: <Instagram size={20} color={BRAND.teal} />, label: "Instagram", value: "@mel.castanares", href: "https://www.instagram.com/mel.castanares?igsh=M2J5eTVqdnhyZ2sx" },
      ...
      <div style={{ background: BRAND.bgDark, padding: "64px 24px 32px", color: "rgba(255,255,255,0.85)" }}>
        <div style={{ maxWidth: 1400, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 48 }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 10, marginBottom: 20 }}>
              <a href="https://www.instagram.com/mel.castanares?igsh=M2J5eTVqdnhyZ2sx" target="_blank" rel="noopener noreferrer"
                style={{ width: 36, height: 36, borderRadius: 8, background: "linear-gradient(135deg, #833ab4, #fd1d1d, #fcb045)", display: "flex", alignItems: "center", justifyContent: "center", transition: "opacity 0.3s" }}
                onMouseEnter={e => (e.currentTarget as HTMLElement).style.opacity = "0.8"}
                onMouseLeave={e => (e.currentTarget as HTMLElement).style.opacity = "1"}>
                <Instagram size={16} color="#fff" />
              </a>
            </div>
            {/* Dream Home Realty Hawai'i Office Logo */}
            <a href="https://www.dreamhomerealtyhawaii.com" target="_blank" rel="noopener noreferrer" style={{ display: "inline-block", opacity: 0.85, transition: "opacity 0.3s" }}
              onMouseEnter={e => (e.currentTarget as HTMLElement).style.opacity = "1"}
              onMouseLeave={e => (e.currentTarget as HTMLElement).style.opacity = "0.85"}>
              <img src="/dream-home-realty-logo.png" alt="Dream Home Realty Hawai'i" style={{ height: 60, width: "auto", objectFit: "contain", filter: "brightness(0) invert(1)" }} />
            </a>
          </div>
        ...
        <div className="footer-bottom" style={{ maxWidth: 1400, margin: "0 auto", marginTop: 48, paddingTop: 24, borderTop: "1px solid rgba(255,255,255,0.08)", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
          ...
          <div style={{ display: "flex", gap: 16, alignItems: "center", flexWrap: "wrap" }}>
            ...
            <a href="https://techsavvyhawaii.com" target="_blank" rel="noopener noreferrer" style={{ fontSize: 11, color: "rgba(255,255,255,0.35)", textDecoration: "none", letterSpacing: "0.04em" }}
              onMouseEnter={e => (e.currentTarget as HTMLElement).style.color = BRAND.tealLight}
              onMouseLeave={e => (e.currentTarget as HTMLElement).style.color = "rgba(255,255,255,0.35)"}>
              Powered by Tech Savvy Hawai'i
            </a>
          </div>
        </div>
      </div>
        ...
        <div style={{
          ...
          <div style={{ padding: "13px 16px", background: `linear-gradient(135deg, ${BRAND.teal}, ${BRAND.tealDark})`, color: "#fff", display: "flex", alignItems: "center", gap: 11, flexShrink: 0 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              <span style={{ fontSize: 11, opacity: 0.75 }}>Online</span>
            </div>
          </div>
              ...
              <div key={i} style={{ display: "flex", justifyContent: msg.role === "user" ? "flex-end" : "flex-start" }}>
                <div style={{
                  ...
                  ...(msg.role === "user"
                    ...
                    : { background: BRAND.bgElevated, color: BRAND.text, borderBottomLeftRadius: 4, border: `1px solid ${BRAND.border}` }),
